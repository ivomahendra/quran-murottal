...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load surah selector dan surah bersamaan
    try {
        await Promise.all([
        ...
        if (autoplay) {
            setTimeout(() => player.playAll(), 1500);
        }
    } catch (err) {
...
async function loadSurah(number) {
    ...
    try {
        ...
        if (bismillah) {
            bismillah.style.display = (number === 9) ? 'none' : 'block';
        }

        // Populate ayah selectors
...
function handleAyahClick(e) {
    ...
    if (target.classList.contains('btn-tafsir-ayah')) {
        openTafsir(
            parseInt(target.dataset.surah),
            parseInt(target.dataset.ayah),
            parseInt(target.dataset.total),
            target.dataset.name
        );
        return;
    }
...
function loadOptions() {
    ...
    if (optRepeat) {
        optRepeat.checked = localStorage.getItem('opt-repeat') === 'true';
        optRepeat.onchange = function() {
            player.repeat = this.checked;
            localStorage.setItem('opt-repeat', this.checked);
        };
        player.repeat = optRepeat.checked;
    }
    ...
    if (optLatin) {
        ...
        optLatin.onchange = function() {
            ...
            showToast('Fitur transliterasi akan segera hadir!');
        };
    }
}
